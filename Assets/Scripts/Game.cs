...
public class Game : MonoBehaviour
{
    ...
    // 2d array for all the positions on the chess board 
    private GameObject[,] positions = new GameObject[8, 8];

    // team for each chess piece 
    ...
    void Start()
    {
        ...
        // Set all piece positions on the poisiton board
    ...
    public GameObject Create(string name, int x, int y)
    {
        ...
        cm.name = name;
        cm.SetXBoard(x); // setting the position
        cm.SetYBoard(y); // setting the position 
        cm.Activate(); // to place correct sprite on board
        return obj;
    }
    ...
    // In order to move chess pieces, make sure position is empty in order to move chess piece there
    public void SetPositionEmpty(int x, int y)
    {
        positions[x, y] = null;
    }

   // Return game object at given position
    public GameObject GetPosition(int x, int y)
    {
        return positions[x, y];
    }

    // check if position given is on board 
