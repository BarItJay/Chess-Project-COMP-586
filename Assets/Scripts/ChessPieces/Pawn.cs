...
public class Pawn : Pieces {
    public override List<Vector2Int> GetAvailableMoves(ref Pieces[,] board, int tileCountX, int tileCountY) {
        ...
        // One in front
        if (IsInBounds(currentX, currentY + dir, tileCountX, tileCountY) && board[currentX, currentY + dir] == null) {
            r.Add(new Vector2Int(currentX, currentY + dir));
        }

        // Two in front
        if (IsInBounds(currentX, currentY + dir, tileCountX, tileCountY) && board[currentX, currentY + dir] == null) {
            if (team == 0 && currentY == 1 && IsInBounds(currentX, currentY + (dir * 2), tileCountX, tileCountY) && board[currentX, currentY + (dir * 2)] == null) {
                r.Add(new Vector2Int(currentX, currentY + (dir * 2)));
            }
            if (team == 1 && currentY == 6 && IsInBounds(currentX, currentY + (dir * 2), tileCountX, tileCountY) && board[currentX, currentY + (dir * 2)] == null) {
                r.Add(new Vector2Int(currentX, currentY + (dir * 2)));
            }
        }

        // Capture
        if (IsInBounds(currentX + 1, currentY + dir, tileCountX, tileCountY) && board[currentX + 1, currentY + dir] != null && board[currentX + 1, currentY + dir].team != team) {
    ...
    private bool IsInBounds(int x, int y, int tileCountX, int tileCountY) {
